...
def _find_error_gate(insns):
    ...
    for i in range(len(insns) - 1):
        ...
        if insns[i].mnemonic == "cmn" and _imm(insns[i], 1) == 1:
            beq = insns[i + 1]
            # The original branch has already been replaced on a retry.
            # Require the result-register compare directly after the call,
            # followed by the same stack-result load before accepting a NOP.
            if beq.mnemonic == "b.eq" or (
                beq.mnemonic == "nop" and i > 0
                and insns[i - 1].mnemonic == "bl"
                and insns[i].reg_name(insns[i].operands[0].reg) == "w0"
                and i + 2 < len(insns) and insns[i + 2].mnemonic == "ldr"
            ):
                return beq
    return None
