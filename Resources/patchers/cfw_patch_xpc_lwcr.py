...
from capstone.arm64_const import ARM64_OP_REG, ARM64_OP_IMM, ARM64_OP_MEM, ARM64_CC_EQ
...
def patch_xpc_lwcr(chunks_dir, *, dry_run=False):
    ...
    if found is None:
        # A previous install may have completed this patch before failing later.
        # Recognize the full matcher-result load/compare and our three edits,
        # within the resolved function, rather than accepting an arbitrary NOP.
        matches = []
        for i in range(3, len(insns) - 2):
            call, load, cmp = insns[i-3:i]
            cset, nop1, nop2 = insns[i:i+3]
            if not (call.mnemonic == "bl" and load.mnemonic == "ldr"
                    and cmp.mnemonic == "cmp" and cset.mnemonic == "cset"
                    and cset.cc == ARM64_CC_EQ and _rn(cset, 0) == "w0"
                    and nop1.mnemonic == nop2.mnemonic == "nop"):
                continue
            if (len(load.operands) != 2 or load.operands[1].type != ARM64_OP_MEM
                    or load.operands[1].mem.disp != 0
                    or len(cmp.operands) != 2
                    or cmp.operands[1].type != ARM64_OP_IMM
                    or cmp.operands[1].imm != 0
                    or _rn(load, 0) != _rn(cmp, 0)):
                continue
            matches.append([cset.address, nop1.address, nop2.address])
        if len(matches) == 1:
            print("      [=] LWCR matcher-result consistency patch already present")
            if not dry_run:
                reattest_modified_pages(chunks, matches[0], dry_run=False)
            return 0
        raise ValueError(
